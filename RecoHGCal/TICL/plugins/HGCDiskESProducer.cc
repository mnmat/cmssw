// -*- C++ -*-
//
// Package:    PlayGround/HGCDiskESProducer
// Class:      HGCDiskESProducer
//
/**\class HGCDiskESProducer
...
class HGCDiskESProducer : public edm::ESProducer {
public:
  HGCDiskESProducer(const edm::ParameterSet&);
  ~HGCDiskESProducer() override;

  using ReturnType = std::unique_ptr<HGCDiskGeomDetVector>;
...
//
HGCDiskESProducer::HGCDiskESProducer(const edm::ParameterSet& iConfig) {
...
HGCDiskESProducer::~HGCDiskESProducer() {
...
void HGCDiskESProducer::makeDisks(int subdet, const CaloGeometry* geom_) {
...
HGCDiskESProducer::ReturnType HGCDiskESProducer::produce(const CaloGeometryRecord& iRecord) {
  ...
  auto product = std::make_unique<HGCDiskGeomDetVector>(disksPos_,disksNeg_);
  return product;
}

//define this as a plug-in
DEFINE_FWK_EVENTSETUP_MODULE(HGCDiskESProducer);